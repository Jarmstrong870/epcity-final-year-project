...
const getHeatingIcon = (description) => {
  ...
  if (description.toLowerCase().includes("boiler"))
    icons.push({ icon: boilerIcon, label: "Boiler" });

  if (description.toLowerCase().includes("radiators"))
    icons.push({ icon: radiatorIcon, label: "Radiator" });

  if (description.toLowerCase().includes("gas"))
    icons.push({ icon: gasCylinderIcon, label: "Gas" });

  if (description.toLowerCase().includes("electric"))
    icons.push({ icon: electricIcon, label: "Electric" });

  if (description.toLowerCase().includes("room heaters"))
    icons.push({ icon: heaterIcon, label: "Heater" });

  if (description.toLowerCase().includes("TRVs"))
    icons.push({ icon: valveIcon, label: "Valve" });

  if (description.toLowerCase().includes("thermostat"))
    icons.push({ icon: thermostatIcon, label: "Thermostat" });

  return icons.length > 0 ? icons : [{ icon: unspecifiedIcon, label: "Unspecified" }];
};
      ...
      <div className={styles.energySection}>
        ...
        <div className={`${styles.energyBox} ${styles.heating}`}>
          ...
          <div>
            <div className={styles.visualisedDescriptions}>
              <div className={styles.heatingIcons}>
                <h4 className={styles.energyH4}>
                  <Link to={`/faq/glossary-page?searchTerm=${encodeURIComponent(t.mainFuel)}`} className={styles.greenQuestionMark}></Link>
                  {t.mainFuel}:
                </h4>
            ...
            <div className={styles.efficiencyStarRatings}
              ...
              <h4 className={styles.energyH4}>
                <Link to={`/faq/glossary-page?searchTerm=${encodeURIComponent(t.mainheatEnergyEfficiency)}`} className={styles.greenQuestionMark}></Link>
            ...
            <div className={styles.efficiencyStarRatings}
              data-value={properties["mainheat_energy_eff"].toLowerCase()}>
              <h4 className={styles.energyH4}><Link to={`/faq/glossary-page?searchTerm=${encodeURIComponent(t.mainheatControllerEnergyEfficiency)}`} className={styles.greenQuestionMark}></Link>
                {t.mainheatControllerEnergyEfficiency}:
              </h4>
          ...
          <div className={styles.iconSection}>
            <div className={styles.heatingIcons}>
              <h4 className={styles.energyH4}>{t.mainheatControllerEnergyEfficiency}:</h4>
          ...
          <div className={styles.visualisedDescriptions}>
            <div className={styles.progressBase}>
              <div className={styles.progressContainer}>
                <h4 className={styles.energyH4}> <Link to={`/faq/glossary-page?searchTerm=${encodeURIComponent(t.lowEnergyLighting)}`} className={styles.greenQuestionMark}></Link>
                  {t.lowEnergyLighting}:
                </h4>
                <progress
              ...
              <div className={styles.efficiencyStarRatings}
                ...
                <h4 className={styles.energyH4}>  <Link to={`/faq/glossary-page?searchTerm=${encodeURIComponent(t.lightingEnergyEfficiency)}`} className={styles.greenQuestionMark}></Link>
                  {t.lightingEnergyEfficiency}:
                </h4>
        ...
        <div className={`${styles.energyBox} ${styles.hotWater}`}>
          ...
          <div className={styles.efficiencyStarRatings}
            ...
            <h4 className={styles.energyH4}> <Link to={`/faq/glossary-page?searchTerm=${encodeURIComponent(t.hotWaterEnergyEfficiency)}`} className={styles.greenQuestionMark}></Link>
              {t.hotWaterEnergyEfficiency}:
            </h4>
            <p>
              {renderStarRating(properties["hot_water_energy_eff"], t)}
            </p>
          </div>
        </div>
      </div>
